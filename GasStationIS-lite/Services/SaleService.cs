    ...
    public static class SaleService
    {
        public static List<FuelSale> GetAll() =>
            DatabaseService.ExecuteReader(
                ...
                "       s.customer_id, c.full_name AS cust_name " +
                "FROM fuel_sales s " +
        ...
        /// <summary>
        /// Регистрирует продажу и уменьшает остаток в резервуаре.
        ...
        public static bool Add(FuelSale sale)
        {
            ...
            // Проверяем и списываем остаток из резервуара
            if (!FuelService.DeductStock(sale.FuelTypeId, sale.Liters))
                return false;

            DatabaseService.ExecuteNonQuery(
                "INSERT INTO fuel_sales " +
                "(fuel_type_id, liters, price_per_liter, total_amount, " +
                " sale_datetime, payment_method, employee_id, customer_id) " +
                "VALUES (@fid, @lit, @ppl, @ta, @dt, @pm, @eid, @cid);",
                cmd =>
                {
            ...
            Logger.Info($"Продажа: {sale.FuelTypeName}, {sale.Liters} л, {sale.TotalAmount:F2} руб.");
            return true;
        }
